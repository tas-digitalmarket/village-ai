...
function setHtml(el, html, fallback = '') {
  if (el) el.innerHTML = html || fallback;
}

function createMetric(label, value, tone = 'primary') {
  return `<div class="metric-pill metric-pill--${tone}"><span>${esc(label)}</span><strong>${esc(value)}</strong></div>`;
}

function createMeter(label, value, tone = 'primary') {
  const safe = pct(value);
  return `<div class="world-meter world-meter--${tone}"><div class="world-meter-head"><span>${esc(label)}</span><strong>${safe}%</strong></div><div class="world-meter-track"><div class="world-meter-fill" style="width:${safe}%"></div></div></div>`;
}
...
function goalMarkup(goal) {
  ...
  return `<article class="goal-row goal-row--${goal.status === 'done' ? 'done' : 'open'}"><div><strong>${esc(goal.title)}</strong><span>${esc(goal.reason || '')}</span></div><em>${done}/${total}</em></article>`;
}

function skillMarkup(name, skill = {}) {
  return `<div class="skill-row"><span>${esc(name)}</span><strong>L${skill.level || 1}</strong><em>${skill.xp || 0} XP</em></div>`;
}

function eventMarkup(event = {}) {
  return `<article class="event-row"><strong>${esc(event.title || 'Event')}</strong><span>${esc(event.note || '')}</span></article>`;
}

function riskMarkup(risk = {}) {
  const top = risk.topRisk;
  if (!top) return '<div class="muted-empty">No active risk.</div>';
  const tone = risk.mode === 'critical' || risk.mode === 'urgent' ? 'warn' : 'ready';
  return `<div class="risk-card risk-card--${tone}"><strong>${esc(top.label || risk.mode)}</strong><span>${esc(top.reason || risk.summary || '')}</span>${createMeter('Severity', risk.overall || top.severity || 0, tone)}</div>`;
}
...
export class HUD {
  ...
  update(data) {
    ...
    if (this.$energyBar) {
      ...
      this.$energyBar.style.background = energy > 40 ? '#31d27c' : energy > 20 ? '#f1c84b' : '#ef6a6a';
    }
    if (this.$energyVal) this.$energyVal.textContent = `${energy}%`;
    if (this.$hungerBar) {
      this.$hungerBar.style.width = `${hunger}%`;
      this.$hungerBar.style.background = hunger < 50 ? '#31d27c' : hunger < 75 ? '#f1c84b' : '#ef6a6a';
    }
    ...
    if (this.$taskContext) {
      const parts = [data.active_goal_title, data.active_task_reason, data.active_risk_id ? `risk: ${data.active_risk_id}` : null].filter(Boolean);
      this.$taskContext.textContent = parts.join(' | ') || 'Waiting for the next meaningful action.';
    }
    ...
    setHtml(this.$skills, [
      skillMarkup('Farming', skills.farming),
      skillMarkup('Animals', skills.animals),
      skillMarkup('Repair', skills.repair),
      skillMarkup('Cooking', skills.cooking),
      skillMarkup('Survival', skills.survival)
    ].join(''));
    ...
    if (this.$memories) {
      ...
      (data.memories || []).slice(0, 5).forEach(memory => {
        ...
        li.textContent = memory.content || memory;
        this.$memories.appendChild(li);
      });
  ...
  updateWorld(world) {
    ...
    this.$world.innerHTML = `
      <div class="world-ledger-grid">
        ${createMetric('Food', storage.food ?? 0)}
        ${createMetric('Wood', storage.wood ?? 0)}
        ${createMetric('Seeds', storage.seeds ?? 0)}
        ${createMetric('Well', `${pct(well.water_level)}%`, pct(well.water_level) <= 20 ? 'warn' : 'primary')}
        ${createMetric('House', `${pct(house.condition)}%`, pct(house.condition) <= 45 ? 'warn' : 'primary')}
        ${createMetric('Animals', `${pct(animals.health)}%`, pct(animals.health) <= 55 ? 'warn' : 'primary')}
        ${createMetric('Motor', `${pct(motorcycle.condition)}%`, pct(motorcycle.condition) <= 40 ? 'warn' : 'primary')}
        ${createMetric('Fuel', `${pct(motorcycle.fuel)}%`, pct(motorcycle.fuel) <= 25 ? 'warn' : 'primary')}
      </div>
      <div class="field-ledger">
        <article><strong>East Field</strong>${createMeter('Moisture', east.moisture, east.moisture < 30 ? 'warn' : 'primary')}${createMeter('Growth', east.growth, east.growth > 75 ? 'ready' : 'primary')}</article>
        <article><strong>West Field</strong>${createMeter('Moisture', west.moisture, west.moisture < 30 ? 'warn' : 'primary')}${createMeter('Growth', west.growth, west.growth > 75 ? 'ready' : 'primary')}</article>
      </div>
    `;
  }
  ...
  updateSchedule(directives) {
    ...
    if (!directives || directives.length === 0) {
      this.$schedule.innerHTML = '<li class="schedule-empty">No upcoming items.</li>';
      return;
    }
  ...
  setConnected(connected) {
    if (!this.$conn) return;
    this.$conn.textContent = connected ? '● Live' : '● Reconnecting';
    this.$conn.className = `dashboard-chip ${connected ? 'dashboard-chip--live' : 'dashboard-chip--warn'}`;
  }
}
